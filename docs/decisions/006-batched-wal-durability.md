...
## Context
PUT/DELETE operations in our KV store are I/O-bound because each write appends a record to the WAL file and flushes to disk immediately. This gives us ~8,000 PUT ops/sec — safe, but slow for workloads that can tolerate bounded data loss.

> **Note:** The WAL format is now binary (CRC32-framed msgpack) as of ADR-007. This decision describes the strict/relaxed durability trade-off, which applies equally to the binary format.
...
### Implementation
- **Buffer**: In relaxed mode, `append()` pushes the encoded binary record into an in-memory list instead of opening the file.
